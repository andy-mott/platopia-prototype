        ...
        <div style={styles.card}>
          <div style={styles.publishedState}>
            ...
            <button style={{ ...styles.primaryBtn, maxWidth: 240 }} onClick={() => {
              setPublished(false); setStep(0); setDuration(null); setCustomHours(""); setCustomMinutes("");
            ...
            <div>
              ...
              <div style={{ ...styles.durationSection, marginBottom: 20 }}>
                ...
                <div style={styles.durationGrid}>
                  {DURATIONS.map((d) => (
                    <button key={d} onClick={() => { setDuration(d); setCustomHours(""); setCustomMinutes(""); }} style={{ ...styles.durationChip, ...(duration === d ? styles.durationChipActive : {}) }}>
                      {formatDurationLabel(d)}
                    </button>
                  ))}
                </div>
                <div style={styles.customDurationRow}>
                  <span style={styles.customDurationLabel}>Or enter custom</span>
                  <input
                    type="number"
                    value={customHours}
                    onChange={(e) => {
                  ...
                  />
                  <span style={styles.customDurationUnit}>hr</span>
                  <input
                    type="number"
                    value={customMinutes}
                    onChange={(e) => {
                    ...
                    style={styles.customDurationInput}
                  />
                  <span style={styles.customDurationUnit}>min</span>
                  {(() => {
                    const h = parseInt(customHours) || 0;
                    const m = parseInt(customMinutes) || 0;
                    const total = h * 60 + m;
                    const valid = total >= 10;
                    const isActive = valid && duration === total && !DURATIONS.includes(duration);
                    return (
                      <button
                        onClick={() => { if (valid) setDuration(total); }}
                        style={{
                          ...styles.customDurationSet,
                          ...(isActive ? styles.customDurationSetActive : {}),
                          ...(!valid ? styles.customDurationSetDisabled : {}),
                        }}
                        disabled={!valid}
                      >
                        {valid ? `Set ${formatDurationLabel(total)}` : "Set"}
                      </button>
                    );
                  })()}
                </div>
...
const styles = {
  ...
  customDurationRow: { display: "flex", alignItems: "center", gap: 8, marginTop: 14, paddingTop: 14, borderTop: "1px solid #eef1f5" },
  customDurationLabel: { fontSize: 13, color: "#7a8a9a", fontWeight: 500, whiteSpace: "nowrap", marginRight: 2 },
  customDurationInput: { width: 56, padding: "8px 6px", borderRadius: 8, border: "1.5px solid #e0e5eb", fontSize: 14, fontWeight: 500, color: "#1a2332", outline: "none", fontFamily: "inherit", background: "#fff", textAlign: "center", boxSizing: "border-box" },
  customDurationUnit: { fontSize: 13, color: "#7a8a9a", fontWeight: 500 },
  customDurationSet: { padding: "8px 16px", borderRadius: 8, border: "1.5px solid #2e86c1", background: "#fff", fontSize: 13, fontWeight: 600, color: "#1a5276", cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap", flexShrink: 0, transition: "all 0.2s", marginLeft: 4 },
  customDurationSetActive: { background: "#eaf4fb", borderColor: "#2e86c1" },
  customDurationSetDisabled: { opacity: 0.35, cursor: "default", borderColor: "#e0e5eb", color: "#9aa5b4" },
